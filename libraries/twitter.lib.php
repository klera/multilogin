...
class TwitterOAuth extends Twitter {
  ...
  public function auth_request($url, $params = array(), $method = 'GET') {
	...
	public function status_update($status, $params = array())
	{
	    ...
	    $url = $this->create_url('statuses/update');
	    try{
	    $values = $this->auth_request($url, $params, 'POST');

	    }
	    catch(TwitterException $e) {
          return   (object)array('error' => $e->getMessage(), 'code'=>$e->getCode()   );
    }
       // dsm($values);
	    return $values;//new TwitterStatus($values);
	}

	public function verify_credentials() {
		$url = $this->create_url('account/verify_credentials');
		$values = $this->auth_request($url, array(), 'GET');
	        //   dsm(print_r($values,1));
	    if (!$values) {
	      return FALSE;
	    }
	    return $values; //new TwitterUser($values);
	}
